...
namespace GeneralCommittee.Infrastructure.Repositories
{
         ...
         ILogger<ArticleRepository> logger) : IArticleRepository
    {
        ...
        /// </remarks>
        public async Task<string> DeleteArticleAsync(Article article)
        {
            ...
            try
            {
                ...
                await dbContext.SaveChangesAsync(); // Ensure changes are saved before committing
             dbContext.Database.CommitTransaction(); // Commit the transaction
        ...
        public async Task<(int, IEnumerable<Article>)> GetAllArticlesAsync(string? search, int requestPageNumber, int requestPageSize , string? sortBy)
        {
            ...
            "date" => baseQuery.OrderBy(r => r.CreatedDate), _ => baseQuery.OrderBy(r => r.Title)  };
        ...
        {
            //TODO : Validate the article parameter
            if (articles == null)
            {
                throw new ArgumentException("Article must not be null.", nameof(articles));
            }
